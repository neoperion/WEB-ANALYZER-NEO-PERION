...
const createLiquidEffect = (
  ...
  return new Effect('LiquidEffect', fragment, {
    ...
    ] as [string, THREE.Uniform<unknown>][]),
  });
};
...
void main(){
  ...
  // Center fade: push pattern to left/right sides, clear the middle
  if (uCenterFade > 0.0) {
    float nx = gl_FragCoord.x / uResolution.x;          // 0 = left, 1 = right
    float distFromCenter = abs(nx - 0.5) * 2.0;         // 0 = center, 1 = edge
    float sideMask = smoothstep(0.0, 1.0, distFromCenter / (1.0 - uCenterFade + 0.001));
    // Also soften top/bottom near center
    float ny = gl_FragCoord.y / uResolution.y;
    float yEdge = min(ny, 1.0 - ny) * 2.0;
    float yFade = smoothstep(0.0, 0.4, yEdge);
    M *= mix(1.0, sideMask, 1.0) * mix(1.0, yFade, (1.0 - distFromCenter) * 0.5);
  }
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      t.uniforms.uCenterFade.value = centerFade;
